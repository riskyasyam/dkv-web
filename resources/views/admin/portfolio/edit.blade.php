...
@section('content')
    <form action="{{ route('admin.portfolio.update', $portfolio) }}" method="POST" enctype="multipart/form-data" class="max-w-2xl">
        @csrf @method('PUT')
        ...
        <div class="bg-white rounded-lg shadow p-8 space-y-6">
            ...
            <div>
                ...
                >
                <p class="mt-1 text-xs text-[#1A1A1A]/60">Dipakai untuk preview videografi di section portfolio.</p>
                @error('youtube_url')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                @if ($portfolio->image)
                    <div class="mb-3 p-3 bg-[#f5f5f5] rounded-lg flex items-center justify-between">
                        <div class="flex items-center gap-2">
                            <svg class="w-5 h-5 text-[#E4572E]" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M4 4a2 2 0 012-2h4.586A2 2 0 0112 2.586L15.414 6A2 2 0 0116 7.414V16a2 2 0 01-2 2H6a2 2 0 01-2-2V4z" clip-rule="evenodd"></path>
                            </svg>
                            <span class="text-sm text-[#1A1A1A]">Gambar tersimpan: {{ basename($portfolio->image) }}</span>
                        </div>
                        <a href="{{ asset('storage/' . $portfolio->image) }}" target="_blank" class="text-xs text-[#E4572E] hover:underline">Lihat</a>
                    </div>
                @endif
                <div class="border-2 border-dashed border-[#e4572e]/20 rounded-lg p-6 text-center hover:border-[#E4572E] transition cursor-pointer" onclick="document.getElementById('image').click()">
                    <svg class="w-12 h-12 mx-auto mb-2 text-[#1A1A1A]/30" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                    </svg>
                    <p id="image-text" class="text-sm font-semibold text-[#1A1A1A]/70">Klik untuk update gambar atau drag & drop</p>
                </div>
                <input 
                    type="file" 
                    id="image" 
            ...
            <div>
                ...
                @if ($portfolio->pdf)
                    <div class="mb-3 p-3 bg-[#f5f5f5] rounded-lg flex items-center justify-between">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-sm text-[#1A1A1A]">PDF tersimpan: {{ basename($portfolio->pdf) }}</span>
                        </div>
                        <a href="{{ asset('storage/' . $portfolio->pdf) }}" target="_blank" class="text-xs text-[#E4572E] hover:underline">Lihat</a>
                    </div>
                @endif
                <div class="border-2 border-dashed border-[#e4572e]/20 rounded-lg p-6 text-center hover:border-[#E4572E] transition cursor-pointer" onclick="document.getElementById('pdf').click()">
                    ...
                    <p id="pdf-text" class="text-sm font-semibold text-[#1A1A1A]/70">Klik untuk {{ $portfolio->pdf ? 'update' : 'upload' }} PDF atau drag & drop</p>
                </div>
                <input 
                    type="file" 
                    id="pdf" 
    ...
    <script>
        function handleImagePreview(input) {
            ...
            document.getElementById('image-text').textContent = fileName || 'Klik untuk update gambar atau drag & drop';
        }

        function handlePdfPreview(input) {
            const fileName = input.files[0]?.name || '';
            document.getElementById('pdf-text').textContent = fileName || 'Klik untuk {{ $portfolio->pdf ? 'update' : 'upload' }} PDF atau drag & drop';
        }
    </script>
@endsection
